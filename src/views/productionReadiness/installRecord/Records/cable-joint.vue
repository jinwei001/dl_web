<template>
  ...
  <div>
    ...
    <div class="content">
      ...
      <div class="table">
        <el-table :data="tableData" style="border: 1px solid #c4c8cf" :cell-style="{borderColor: '#c4c8cf'}"
          :header-cell-style="{'background':'#cddce6',borderColor: '#c4c8cf'}" border fit highlight-current-row>
    ...
    <div v-if="isShowDialog" class="sc-sort-Dialog">
      <el-dialog v-dialogDrag :visible.sync="isShowDialog" :title="`${$dl}接头安装记录`" top="20vh" :center="true" :before-close="handleClose">
          ...
          <div class="Form-input">
            <div class="form-long-item">
              ...
              <a href="javascript:;" class="a-upload">
                <input type="file" id="upload" name="FileUpload" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel"
                  @change="changeImage($event)" />{{this.fileName}}
              </a>
            </div>
          </div>
        </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      DelFunOne(val) {
        ...
        dljtremove(qs.stringify(list)).then(res => {
          // //console.log(res.data);
          if (res.data.code == 0) {
            this.$message({
