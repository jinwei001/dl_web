<template>
  ...
  <div>
    ...
    <div v-if="isShowDialog" class="sc-sort-Dialog">
      <el-dialog v-dialogDrag :visible.sync="isShowDialog" :title="`单芯${$dl}箱安装记录`" top="20vh" :center="true" :before-close="handleClose">
...
export default {
  ...
  methods: {
    ...
    DelFunOne(val) {
      ...
      dxdlxremove(qs.stringify(list)).then(res => {
        // //console.log(res.data);
        if (res.data.code == 0) {
          this.$message({
