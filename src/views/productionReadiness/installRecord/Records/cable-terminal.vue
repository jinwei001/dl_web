...
import { download } from "@/api/productionReadiness/export";
import {
  dlzdlist,
  gclist,
  dlzdadd,
...
export default {
  ...
  methods: {
    ...
    DelFunOne(val) {
      ...
      dlzdremove(qs.stringify(list)).then(res => {
        // //console.log(res.data);
        if (res.data.code == 0) {
          this.$message({
