    ...
    >
      ...
      >
        ...
        <div class="Form-input">
          ...
          <!-- <el-form-item label="通道建设show类型：" label-width="130px" style="width:33%">
            <el-select
              v-model="addlist"
              placeholder="请输入通道建设类型"
              @change="chooseTDlxshow"
              filterable
              multiple
              value-key="bzid"
              style="width:100%" 
        ...
        <div class="Form-input">
          ...
          <!-- <el-form-item label="负责班组：" label-width="130px" prop="fzbzid" style="width:33%"> -->
...
export default {
  ...
  data() {
    return {
      ...
      //     }
      //   ],
      //   fzbz: [{ required: true, message: "请输入负责班组", trigger: "change" }]
      // },
      wjmc: null,
      wjlj: null,
  ...
  created() {
    ...
    if (this.isAdd) {
       ...
       * 通道建设补充意见列表
       */
      BcyjList(qs.stringify({ tdjsid: this.sopenId, sgys: 1 })).then(res => {
        // //console.log(res);
        this.bcyj = res.data.rows;
      });
      YjList(qs.stringify({ tdjsid: this.sopenId, sgys: 1 })).then(res => {
        // //console.log(res);
        // this.gkk_table_list = res.data.data;
        ...
        //console.log(this.gkk_table_list, "!!!!!!shuj");
      });
      ...
      info.LClist(qs.stringify({ tdjsid: this.sopenId, sgys: 1 })).then(res => {
        this.lctable = res.data.rows;
      });
      /**附件列表 */
      ExportFn(
        "/cable/tZjgcFj/list",
        qs.stringify({ zbid: this.sopenId, jd: 5 })
      )
        .then(res => {
  ...
  methods: {
    ...
    AddnewBc() {
      ...
      this.bcyj.push({ sxh: time, sgys: "1" });
    },
    /**选中补充意见 */
    CheckFun(val) {
      //console.log(val);
    ...
    SuerFun() {
      this.$refs["formrules"].validate(val => {
        if (val) {
          ...
          info.aad(qs.stringify(this.addlist)).then(res => {
    ...
    TGFn() {
      ...
      var thlist = {
        ...
        jdmc:
          this.lctable.length > 0
            ? this.lctable == null || this.lctable
              ? "1"
              : this.lctable[this.lctable.length - 1].jdmc
            : "1",
        clrid: "",
