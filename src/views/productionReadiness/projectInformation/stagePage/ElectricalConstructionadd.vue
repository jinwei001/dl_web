...
<template>
  <div class="hastitle">
      ...
      >
        ...
        <div v-for="(item,i) in gkk_table_list" :key="i">
          ...
          >
            ...
            <el-table-column prop="scyj" label="审批意见" align="center" width="180">
              <template slot-scope="scope">
            ...
            <el-table-column label="操作" align="center" width="350">
              <template slot-scope="scope">
                ...
                <el-button size="small" @click="pic(scope.row)">图片</el-button>
    ...
    <div v-if="isshowGKKPic">
      <picpicgkk @childFn="parentGKKpicFn" :openobj="openobj" />
    </div>
  </div>
</template>
...
import picpicgkk from "./Add_tdjs_pic_show";
import {
  GKK_more,
  GC,
  UserList,
...
export default {
  ...
  data() {
    return {
      ...
      //   dlcj: [{ required: true, message: "请输入DL厂家", trigger: "blur" }],
      //   dlxh: [{ required: true, message: "请输入DL型号", trigger: "blur" }],
      ...
      //   ],
      //   ysdlfsd: [
      //     { required: true, message: "请输入验收DL敷设段", trigger: "blur" }
      //   ],
      //   ysdlfj: [
      //     { required: true, message: "请输入验收DL附件位置", trigger: "blur" }
      //   ],
      //   dqsglxid: [
      //     {
      //       required: true,
  ...
  created() {
    ...
    if (this.isAdd) {
      ...
      GC(qs.stringify({ objId: this.openID })).then(res => {
        ...
        // this.addlist.yj = list.yj;
        this.addlist.gcmc = list.gcmc;
        this.addlist.gcxz = list.gcxz;
        this.addlist.gcsx = list.gcsx;
        this.addlist.dqsglxid = ["1"];
      });
    } else {
      getList(qs.stringify({ objId: this.sopenId, sgys: 1 })).then(res => {
        // //console.log(res);
        let data = res.data.rows[0];
        // (this.addlist.yj = data.yj), this.addlist.yj = null
      ...
      info
        .BcyjList(qs.stringify({ dqsgid: this.sopenId, sgys: 1 }))
        .then(res => {
          // //console.log(res);
          this.bcyj = res.data.rows;
        });
      info.LClist(qs.stringify({ dqsgid: this.sopenId, sgys: 1 })).then(res => {
        this.lctable = res.data.rows;
      });
      info.YjList(qs.stringify({ dqsgid: this.sopenId, sgys: 1 })).then(res => {
        // //console.log(res);
        // this.gkk_table_list = res.data.data;
      ...
      ExportFn(
        "/cable/tZjgcFj/list",
        qs.stringify({ zbid: this.sopenId, jd: 6 })
      )
        .then(res => {
  ...
  methods: {
    pic(val) {
      ...
      this.isshowGKKPic = true;
    },
    parentGKKpicFn(val) {
      this.isshowGKKPic = false;
    },
    ...
    tjbz(val) {
      ...
      this.submitList.bzid = val.objId;
      this.submitList.bzmc = val.groupname;
      /**当前处理人 */
    ...
    tjczr(val) {
      ...
      this.submitList.clrmc = val.name;
      this.submitList.clrid = val.objId;
    ...
    SuerFun() {
      this.$refs["formrules"].validate(val => {
        if (val) {
          ...
          this.addlist.dqsglxid = this.addlist.dqsglxid.toString();
          ...
          this.gkkList.forEach((o, j) => {
            this.$set(o, "sxh", j + 1);
          });
          this.addlist.lcdata = JSON.stringify(this.lcdata); //流程履历数据
