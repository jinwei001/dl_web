    ...
    >
      ...
      >
        ...
        <div class="Form-input">
          ...
          <el-form-item label="工程名称：" prop="gcmc" label-width="130px" style="width:66.6%">
            <el-input v-model="addlist.gcmc" clearable readonly />
          </el-form-item>
        </div>
        <div class="Form-input">
          <el-form-item label="工程性质：" prop="gcxz" label-width="130px" style="width:33%">
            <el-input v-model="addlist.gcxz" clearable readonly />
          </el-form-item>
          <!-- <el-form-item label="工程属性：" prop="gcsx" label-width="130px" style="width:33%">
            <el-input v-model="addlist.gcsx" clearable readonly />
          </el-form-item>-->
          <el-form-item :label="`${$dydj}：`" label-width="130px" style="width:33%">
            <el-input v-model="addlist.dydj" clearable />
          </el-form-item>
          <el-form-item style="width:33%" />
        </div>
        <div class="Form-input">
          <el-form-item label="建设方：" prop="jsf" label-width="130px" style="width:33%">
            <el-input v-model="addlist.jsf" clearable />
          </el-form-item>
          <el-form-item :label="`${$dl}设计长度（米）`" prop="dlsjcd" label-width="180px" style="width:33%">
            <el-input v-model="addlist.dlsjcd" clearable />
          </el-form-item>
        ...
        <div class="Form-input">
          <el-form-item label="项目经理：" prop="sjtjch" label-width="130px" style="width:33%">
            <el-input v-model="addlist.sjtjch" clearable />
          </el-form-item>
          <el-form-item label="审查时间：" prop="scsj" label-width="130px" style="width:33%">
            <el-date-picker
              v-model="addlist.scsj"
              type="date"
              value-format="yyyy-MM-dd"
              placeholder="请选择日期"
            />
          </el-form-item>
          <el-form-item label="负责班组：" prop="bzmc" label-width="130px" style="width:33%">
            <el-select
        ...
        <div class="Form-input">
          ...
          <el-form-item style="width:33%"></el-form-item>
        </div>
...
import { GC, GKK_view, UserList, GropList } from "@/api/public";
...
export default {
  ...
  data() {
    return {
      ...
      //   xlmc: [{ required: true, message: "请输入"+${$xl}+"名称", trigger: "blur" }],
      ...
      //   dlsjcd: [
      //     { required: true, message: "请输入"+this.$dl+"设计长度", trigger: "blur" }
      //   ],
      //   scry: [{ required: true, message: "请输入审查人员", trigger: "blur" }],
      //   sjtjch: [
      //     { required: true, message: "请输入项目经理", trigger: "blur" }
      //   ],
      //   dlgm: [{ required: true, message: "请输入"+this.$dl+"规模", trigger: "blur" }],
      ...
      //   ],
      //   bzmc: [{ required: true, message: "请输入负责班组", trigger: "change" }]
      // },
      wjmc: null,
      wjlj: null,
      objIdson: null,
  ...
  watch: {
    ...
    openID(val) {
      this.groundopenid = val;
    },
    sopenId(val) {
      this.clearsopenId = val;
      this.groundsonopneid = val;
    }
  },
  created() {
    ...
    if (this.isAdd) {
      ...
      /**新增的管控卡 */
      GKK_view(qs.stringify({ jdid: 3 })).then(res => {
      ...
      if (this.editbtn) {
        this.title = "施工图审查-处理流程";
      } else {
        this.title = "施工图审查-查看流程";
      }
      /**查询设计初步设计补充意见列表 */
      BcyjList(qs.stringify({ sgtscid: this.sopenId })).then(res => {
        //console.log(res);
        this.bcyj = res.data.rows;
      });
      LClist(qs.stringify({ sgtscid: this.sopenId })).then(res => {
        //console.log(res);
        this.lctable = res.data.rows;
      });
      GKK(qs.stringify({ sgtscid: this.sopenId })).then(res => {
        //console.log(res);
        this.gkktable = res.data.rows;
      });
      /**工程内容 */
      ...
      ExportFn(
        "/cable/tZjgcFj/list",
        qs.stringify({ zbid: this.sopenId, jd: 3 })
      )
        .then(res => {
