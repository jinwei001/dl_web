  ...
  <div class="long-hastitle">
    ...
    >
      ...
      <div v-show="loading">
        ...
        <!--模板数据-->
        <div v-for="(items,index) in riskDivisorData" style="width: 100%;overflow: auto">
          <div v-for="(item,i) in items.list">
        ...
        <table
          v-show="riskDivisorData"
          class="modelTableStyle"
          border="0"
          cellpadding="0"
          style="width: 100%;"
        >
          ...
          <tbody>
            <tr class="firstLine">
              ...
              <!-- <td>{{riskResult.riskStatus}}</td> -->
               <td>{{max11(maxjs,riskResult.shouldDeductTotal)}}</td>
    ...
    >
      <div class="Form-input">
        <div class="form-long-item">
          ...
          <el-input v-model="diaLogForm.pointsReason" clearable readonly />
        </div>
      </div>
      <div class="Form-input">
        <div class="form-long-item">
          <label class="form-label" style="width: 30%;">现场排查情况：</label>
          <el-input v-model="diaLogForm.sceneCheckCondition" clearable readonly />
        </div>
      </div>
      <div class="Form-input">
...
export default {
  ...
  methods: {
    ...
    // 获取通道类型
    getChannelType() {
      this.channelIdOnce = this.value; // 选择的通道id
      channelType(this.channelIdOnce).then(response => {
        ...
        if (response.status == 200) {
          ...
          this.riskDivisorParams.channelId = this.value;
