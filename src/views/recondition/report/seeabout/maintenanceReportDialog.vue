...
export default {
  ...
  methods: {
    ...
    getysr() {
      console.log(111);
      getnextuser(this.$qs.stringify({ groupid: this.form.ysbz })).then(res => {
        this.selection.ysr = res.data.rows;
      });
    },
    // 获取回显内容
    ...
    // 保存提交按钮
    save() {
      this.form.maintenanceacceptance = 3;
    ...
    noAgree() {
      this.form.maintenanceacceptance = 2;
      edit(this.$qs.stringify(this.form)).then(result => {
        if (result.data.code == 0) {
