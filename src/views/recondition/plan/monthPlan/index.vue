          ...
          <el-form>
            <el-col>
              <el-form-item style="float: left;">
                ...
                <el-button type="primary" @click="generateNextPlan">生成周检修计划</el-button>
                <el-button type="primary" @click="generateTaskJob">生成工作任务</el-button>
              </el-form-item>
            </el-col>
          </el-form>
          <div class="table">
...
import {
  ...
  generateTaskJob,
  exportPlan
} from "@/api/recondition/monthPlan";
export default {
  components: {
    Pagination,
    addView,
    ...
    generateTaskJob() {
      if (this.CheckedSelection.length > 0) {
        ...
        if (ids.length == this.CheckedSelection.length) {
          ...
          var chosenId = ids.join(); // 数组转字符串
          this.form.ids = chosenId;
          generateTaskJob(this.$qs.stringify({ ids: this.form.ids })).then(
            res => {
              if ((res.data.code == 0)) {
                this.$message({
                  message: "生成下次检修计划成功",
