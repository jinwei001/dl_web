      ...
      <div class="content">
        ...
        <div class="table">
          <el-table :data="tableData" style="border: 1px solid #c7c7c7" :cell-style="{borderColor: '#c7c7c7'}"
            ...
            <el-table-column align="center" label="操作" class-name="small-padding fixed-width" min-width="300" fixed="right">
              <template slot-scope="scope">
                <el-col style="text-align: center;">
                  <el-button class="onSearch" size="small" @click="ViewFun(scope.row)">查看流程</el-button>
                  <el-button class="onSearch" size="small" @click="editFun(scope.row)" v-if="scope.row.currentnode==='5'">计划管理</el-button>
                </el-col>
              </template>
            </el-table-column>
          </el-table>
...
<script>
 ...
 message: this.$dydj+this.$dl+'下拉列表获取失败',
      ...
      dianxingfun() {
        ...
        tHiddendangerMessagetoTypical(qs.stringify(data)).then(response => {
     if (response.status == 200) {
       if(response.data.msg=='操作成功'){
         this.$message({
           message: response.data.msg,
