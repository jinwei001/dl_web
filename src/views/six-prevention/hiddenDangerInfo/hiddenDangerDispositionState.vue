  ...
  <div class="main-body">
    <div class="main-userSelect">
      <div class="searchForm_glf">
        <el-form ref="ruleForm" inline>
          <el-form-item>
            <el-select v-model="ruleFrom.disposeState" clearable placeholder="请选择">
              <el-option
                v-for="item in disposeStateArr"
                :key="item.id"
                :label="item.value"
                :value="item.id"
              />
    ...
    <div class="content">
      <div class="leftWarp">
        ...
        >
          <el-table-column property="chuzZhiZhuangTaiName" :show-overflow-tooltip="true" label="处置状态" />
          <el-table-column property="faXianBanZuName" :show-overflow-tooltip="true" label="发现班组" />
      ...
      <div class="allmapBox">
        ...
        <div v-if="dialogVisible" class="markerBox">
          <div class="detailTitle">
            <div class="titleType">{{ hiddenState }}</div>
            <div class="guanbi" @click="handleClose">X</div>
          </div>
          <div class="detailContent">
            <div class="contentItem">隐患处置状态：{{ itemObj.chuzZhiZhuangTaiName }}</div>
...
export default {
  ...
  props: ['hiddenDangerDispositionState'],
  ...
  data() {
    return {
      ...
      hiddenState: this.hiddenDangerDispositionState,
  ...
  methods: {
    ...
    // 处置状态下拉框
    async getHiddenStateSelectList() {
      var that = this
      const res = await getHiddenTypeSelect()
      var result = res.data
      that.disposeStateArr = result[2].chuLiZhuangTai
    },
    // 获取地图坐标点
