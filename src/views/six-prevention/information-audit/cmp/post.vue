...
<template>
  ...
  <div class="xs-hastitle">
    <el-dialog v-dialogDrag :visible.sync="showDialog" title="编辑" :center="true" :before-close="handleClose">
      <el-form
        id="SHBOX"
        ref="form"
        :model="form"
      ...
      >
        <!--      ----------------------------------步骤条-------------------------------------- -->
        <el-steps :active="0" align-center finish-status="success" style="margin-bottom: 20px;">
          <el-step v-for="(step,index) in steplist" :key="index" :title="step" />
        </el-steps>
        <!--      ----------------------------------列表-------------------------------------- -->
        <div v-if="submitReviewparameter.starts==0">
            ...
            <el-row>
              ...
              <el-col :span="8">
               <el-form-item ref="cableObj[0]" prop="cableObj[0]" :label="$dl">
                 <el-select v-model="form.cableObj" placeholder="请选择" :disabled="daidianshow===true" value-key="dlid"  multiple>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item ref="Hiddendangerdescription" prop="Hiddendangerdescription" label="隐患描述">
                  <el-input v-model="form.Hiddendangerdescription" clearable type="textarea" :rows="7" placeholder="请输入隐患描述" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-col>
        ...
        <el-col :span="24" />
        <el-col v-if="submitReviewparameter.starts==1" :span="12">
          <el-form-item prop="banzhu" label="指定班组">
            <el-select v-model="form.banzhu" clearable placeholder="请选择" style="width: 80%;" @change="zhidingFXbzchangeFun">
              <el-option v-for="(item,index) in selection.banZu" :key="index" :label="item.value" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col v-if="submitReviewparameter.starts==1" :span="12">
          <el-form-item prop="uselist" label="下一审批人">
            <el-select v-model="form.uselist" clearable placeholder="请选择" style="width: 80%;" filterable>
              <el-option v-for="item in sevdata.ZDrenlist" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-form>
      <div slot="footer" class="dialog-footer" style="margin-top: 100px;">
        <el-button class="UserSearchFun" @click="submit">提交</el-button>
...
export default {
  ...
  mounted() {
    console.log(this.submitReviewparameter.data.hdeq,'LLLLL')
    // this.form.VoltageLevel = this.submitReviewparameter.data.voltage // DYDJ
    this.YHxianluchangeFun(this.submitReviewparameter.data.lineid)
    this.form.HiddenDangerLine = this.submitReviewparameter.data.lineid.split(',') // 隐患XL
    // let _this=this
    // setTimeout(function() {
    ...
    // }, 1000)
    this.form.cableObj= this.submitReviewparameter.data.cableJson // 电
    this.form.HiddenEquipmentType = this.submitReviewparameter.data.hdeqtype // 设备类型
    this.getSBmingchengList()
    this.form.HiddenEquipment = this.submitReviewparameter.data.hdeq===null?'':this.submitReviewparameter.data.hdeq.split(',') // 隐患设备名称
    this.form.HiddenPart = this.submitReviewparameter.data.hdposition // 隐患部位
    // ---------------------------------------------
    this.form.DiscoveryMethod = this.submitReviewparameter.data.findtype // 发现方式
    this.form.DiscoveryUnit = this.submitReviewparameter.data.findunit // 发现单位
    this.form.date_time = this.submitReviewparameter.data.finddate // 发现日期
    this.form.DiscoveryTeam = this.submitReviewparameter.data.findbz // 发现班组
    this.FXbzchangeFun(this.form.DiscoveryTeam)
    this.form.FindPeople.name = this.submitReviewparameter.data.finduser // 发现人
    this.form.FindPeople.id = this.submitReviewparameter.data.finduserid // 发现人
    // ---------------------------------------------
    this.form.HiddenDangerLocation = this.submitReviewparameter.data.findaddress // 隐患地址
    this.form.Hiddendangerlocationlongitude = this.submitReviewparameter.data.longitude // 隐患经度
    this.form.Hiddendangerlocationdimension = this.submitReviewparameter.data.latitude // 隐患维度
    this.form.Sixtypeshiddendangers = this.submitReviewparameter.data.sixhdtype // 六防隐患类型
    this.form.Hiddendangertype = this.submitReviewparameter.data.hdtype // 隐患类型
    this.gradeDatachange(this.submitReviewparameter.data.hdclass)
    this.form.Hiddendangerlevel = this.submitReviewparameter.data.hdclass // 隐患等级
    // ---------------------------------------------
    this.form.HiddenPrimitiveUnit = this.submitReviewparameter.data.hdcompany // 隐患地址
    this.form.Hiddencontact = this.submitReviewparameter.data.hdcontacts // 隐患经度
    this.form.contactnumber = this.submitReviewparameter.data.telephone // 隐患维度
    this.form.ConstructionStatus = this.submitReviewparameter.data.construction // 六防隐患类型
    this.form.Hiddendangerdescription = this.submitReviewparameter.data.hdreason // 隐患类型
    this.form.sbremarks = this.submitReviewparameter.data.sbremarks // 隐患等级
    this.BasicInformationlistfun() // 获取图片编辑需要的详情方法
    if (this.submitReviewparameter.data.ddjcid) {
  ...
  methods: {
    ...
    BasicInformationlistfun() {
      const data = {
        objId: this.submitReviewparameter.data.objId
      }
    ...
    submit() {
      this.$refs.form.validate((valid, object) => {
        if (valid) {
          var data = {}
          if (this.submitReviewparameter.starts === 0) {
            // var arry = []
            ...
            data = {
              ...
              // cableId:this.form.cableObj.dlid||'',//DLID
              ...
              ddjcid: this.submitReviewparameter.data.ddjcid
            }
          }
          if (this.submitReviewparameter.starts === 1) {
            data = {
              objId: this.submitReviewparameter.data.objId,
              currentprocessor: this.form.uselist,
              currentbz: this.form.banzhu,
              isstart: 1
            }
          }
