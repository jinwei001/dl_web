      ...
      <el-form ref="dataForm" :model="form" label-position="left" style="width:100% ;margin: 0 auto;">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="required-ruleForm thr">
          ...
          <el-form-item label="通道名称:">
            <el-select v-model="ruleForm.tdId" filterable clearable placeholder="请选择" :filter-method="filterList" disabled>
...
import {
  ...
  downloadImg,
  proList,
  channelsAddForm, // form 表单提交
  xsjlListAdd,
  xsjlInputChannelCheck,
  tdxsjlListCheck as xsjlListCheck,
  tdresultEditPro as editPro, //
  channelsEditForm as editform
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        tdId: [{ required: true, message: '请选择通道', trigger: 'blur' }],
        opsGroupId: [
        ...
        patrolUserId: [
          { required: true, message: '请选择巡视人', trigger: 'change' }
        ],
        zxsj:[
  ...
  created() {
    ...
    this.filterList()
    if (this.isEdit) {
      xsjlInputChannelCheck(this.openID).then(res => {
        let resFormData = res.data.data
        this.ruleForm.patrolPlanTime = resFormData.jhxsrq
        this.ruleForm.tdId = resFormData.tdId
        this.ruleForm.opsGroupId = resFormData.xsbzId
        this.ruleForm.dlId = resFormData.dlId
        this.ruleForm.patrolUserId = resFormData.xsrId
        this.ruleForm.zxsj = resFormData.zxsj
      })
      ...
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          if (!this.isEdit) {
            ...
            Promise.all([
              xsjlListAdd({
                projectArray: JSON.stringify(this.InspectionRecord)
              }),
              channelsAddForm({ ...this.ruleForm, xsjgId: this.openID ,photoUrl: this.imgArr.join(','),picData:JSON.stringify(this.picData)})
            ]).then(res => {
              let val1 = res[0].data
              let val2 = res[1].data
            ...
            Promise.all([
              ...
              editform({
                ...this.ruleForm,
                xsjgId: this.openID,
                photoUrl: this.imgArr.join(',')
                ,picData:JSON.stringify(this.picData)
              })
            ]).then(res => {
              let val1 = res[0].data
              let val2 = res[1].data
