<template>
  ...
  <div class="xs-hastitle whitedisable_mm">
    ...
    <el-dialog :title="title" :visible.sync="allshowDialog" :close-on-click-modal="false" width="1060px" center :before-close="handleClose">
        ...
        <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-width="140px">
          ...
          <div class="flexThird">
            ...
            <el-form-item label="消缺时间" prop="xqTime" class="mm_alart">
              <el-date-picker v-model="ruleForm2.xqTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" :disabled='thisStatus>=2'></el-date-picker>
            </el-form-item>
            <el-form-item label="工作负责人" prop="workDutyPerson" class="mm_alart">
              <el-input v-model="ruleForm2.workDutyPerson" :disabled='thisStatus>=2'></el-input>
            </el-form-item>
            <el-form-item label="是否集中处理" prop="ifFocusHandle" class="mm_alart">
              <el-select v-model="ruleForm2.ifFocusHandle" class="form-item-select" :disabled='thisStatus>=2'>
                <el-option label="是" value="1"></el-option>
                <el-option label="否" value="0"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="是否延迟消缺" prop="ifDelay" class="mm_alart">
              <el-select v-model="ruleForm2.ifDelay" class="form-item-select" :disabled='thisStatus>=2'>
                <el-option label="是" value="1"></el-option>
                <el-option label="否" value="0"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="延期原因" prop="delayReason" style="width:100%" class="mm_alart">
              <el-input v-model="ruleForm2.delayReason" type="textarea" :disabled='thisStatus>=2'></el-input>
            </el-form-item>
            <el-form-item label="缺陷处置方案" prop="approvalAdvise" style="width:100%" class="mm_alart" v-if='thisStatus<2'>
            ...
            <el-form-item label="缺陷处置方案" style="width:100%" class="mm_alart" v-else>
              <el-button type="primary" class="uploadBtn" :disabled="!thisDetail.handlePlan" @click="downs(publicIp + thisDetail.handlePlan,'处理附件资料')">点击下载</el-button>
            </el-form-item>
            <el-form-item label="缺陷处置结果" prop="handleResult" style="width:100%" class="mm_alart">
              <el-input v-model="ruleForm2.handleResult" type="textarea" :disabled='thisStatus>=2'></el-input>
            </el-form-item>
          ...
          <div class="imgCon picCon">
            ...
            <el-form-item class="addPic" v-if='thisStatus<2'>
          ...
          <el-form-item label="遗留问题" prop="remainProblem" style="width:100%" class="mm_alart">
            <el-input v-model="ruleForm2.remainProblem" type="textarea" :disabled='thisStatus>=2'></el-input>
          </el-form-item>
          <el-form-item label="处理附件资料" style="width:100%" class="mm_alart" v-if='thisStatus<2'>
          ...
          <el-form-item label="处理附件资料" style="width:100%" class="mm_alart" v-else>
            <el-button type="primary" class="uploadBtn"  :disabled="!thisDetail.handlePlan" @click="downs(publicIp + thisDetail.annex,'处理附件资料')"> 点击下载</el-button>
          </el-form-item>
        </el-form>
      </div>
      <!-- -----------验收-------------- -->
      <div v-if='thisStatus>=2'>
        <el-form :model="ruleForm3" :rules="rules3" ref="ruleForm3" label-width="140px">
          ...
          <div class="flexThird">
            ...
            <el-form-item label="验收是否合格" prop="ifQualified" class="mm_alart">
              <el-select v-model="ruleForm3.ifQualified" class="form-item-select" :disabled='thisStatus>=3'>
                <el-option label="是" value="1"></el-option>
                <el-option label="否" value="0"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="验收时间" prop="acceptTime" class="mm_alart">
              <el-date-picker v-model="ruleForm3.acceptTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" :disabled='thisStatus>=3'></el-date-picker>
            </el-form-item>
            <el-form-item label="汇报调度情况" prop="dispatchSituation" class="mm_alart">
              <el-input v-model="ruleForm3.dispatchSituation" :disabled='thisStatus>=3'></el-input>
            </el-form-item>
            <el-form-item label="汇报监控情况" prop="monitorSituation" class="mm_alart">
              <el-input v-model="ruleForm3.monitorSituation" :disabled='thisStatus>=3'></el-input>
            </el-form-item>

            <el-form-item label="验收意见" prop="acceptOpinion" style="width:100%" class="mm_alart">
              <el-input v-model="ruleForm3.acceptOpinion" type="textarea" :disabled='thisStatus>=3'></el-input>
            </el-form-item>
            <el-form-item label="备注" prop="acceptRemark" style="width:100%" class="mm_alart">
              <el-input v-model="ruleForm3.acceptRemark" type="textarea" :disabled='thisStatus>=3'></el-input>
            </el-form-item>
          </div>
          <h3 class="modelTitle" style="width:100%">上传照片</h3>
          <div class="imgCon picCon">
      ...
      <div slot="footer" class="dialog-footer" v-if='this.thisStatus<3'>
...
export default {
  ...
  methods: {
     ...
     */
    SuerFun() {
      this.$refs['ruleForm' + (this.thisStatus + 1)].validate(valid => {
        if (valid) {
          let commitData = {}
          if (this.thisStatus == 3) {
            this.ruleForm2.afterPhotoUrl = this.afterPhotoUrlArr.join(',')
            commitData = {
              ...this['ruleForm' + (this.thisStatus + 1)],
            ...
            if (this.ruleForm3.ifQualified == 0) delete commitData.xqStatus
          }
