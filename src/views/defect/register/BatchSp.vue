<template>
  ...
  <div class="xs-hastitle whitedisable_mm">
    <el-dialog :visible.sync="showDialog" title="开启流程" v-dialogDrag :center="true" width="50%" :before-close="handleClose">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="140px" class=" flexThird">
    ...
    SuerFun() {
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          let loginUserId = this.getCookie('userId')
          let approvalResult = 1
          let qxId = this.qxIdArr.join(',')
          console.log({ ...this.ruleForm, loginUserId, approvalResult, qxId })
          registerBatch({
            ...this.ruleForm,
            loginUserId,
            approvalResult,
