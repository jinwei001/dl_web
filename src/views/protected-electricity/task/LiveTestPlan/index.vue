        ...
        <el-form>
          <el-col>
            <el-form-item style="float: left;">
              <el-button class="onSearch" @click="Addfun" size="small">
                <i class="el-icon-plus"> 新增</i>
              </el-button>
              <el-button type="danger" plain @click="DelAllFn" size="small">
                <i class="el-icon-close"></i> 删除
              </el-button>
<!--              <el-button class="onSearch" @click="shangchengfun" size="small">
                <i class="el-icon-plus"> 生成带电检测计划</i>
              </el-button> -->
              <el-button class="onSearch" @click="backfun" icon="el-icon-back" style="text-align: right;" size="small">
                后退
              </el-button>
            </el-form-item>
          </el-col>
      ...
      <div class="table">
        <el-table :data="tableData" height="600" style="border: 1px solid #c7c7c7;width:100%" :cell-style="{borderColor: '#c7c7c7'}"
          ...
          <el-table-column :label=" $xl+'名称'" align="center" prop="xlmc" width="150" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column label="检测班组" align="center" prop="jcbz" width="160"></el-table-column>
          <el-table-column label="检测人" align="center" prop="jcr" width="160"></el-table-column>
          <el-table-column label="检测单位" align="center" prop="jcdw" width="160"></el-table-column>
          <el-table-column align="center" label="操作" class-name="small-padding fixed-width" width="230">
            <div slot-scope="scope" class="">
              <el-button class="onSearch"  @click="editFun(scope.row.id)" size="small">编辑</el-button>
              <el-button plain  type="danger" @click="del(scope.row.id)" size="small">删除</el-button>
    ...
    <div v-if="isShowDialog">
      <Add :selection='selection' @childFn="parentFn" :isEdit="isEdit" :openID="openID" :Taskidname='Taskidname' :lineid='lineid'/>
    </div>
  ...
  export default {
    ...
    created() {
      ...
      linesSelect().then(res => {
        // XL
       //保电任务表选中线路和线路列表进行对比    
        this.selection.linesSelectArr=[]
        for(let i=0;i<res.data.rows.length;i++){
          for(let s=0;s<JSON.parse(this.Taskidname.bdxl).length;s++){
            if(JSON.parse(this.Taskidname.bdxl)[s].id===res.data.rows[i].id){
                 this.selection.linesSelectArr.push(res.data.rows[i])
              }
          }
