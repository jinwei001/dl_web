  ...
  <div class="xs-hastitle">
    <el-dialog :visible.sync="showDialog" :title="`${$xl}巡视计划管理`" v-dialogDrag :center="true" :before-close="handleClose">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="required-ruleForm thr">
        <!-- //XL -->
          <el-form-item :label="$xl+'名称'" class="el-form-itemstyle" prop="xlId">
          <el-select v-model="ruleForm.xlId" clearable placeholder="请选择"  value-key="id">
            <el-option v-for="item in JSON.parse(selection.linesArr)" :key="item.id" :label="item.value" :value="item.id"></el-option>
          </el-select>
          </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        lastPlanTime: [{ required: true, message: '请选择上次计划来源', trigger: 'change' }]
      },
      form: {
        sbmc: '',
        date_time: null,
  ...
  methods: {
    ...
    getJcrList(groupobj) {
      ...
      if(groupobj){
        ...
        this.ruleForm.groupName=groupobj.groupname

        jcrSelect(this.ruleForm.groupName).then(res => {
          // DL
    ...
    getLine(e) {
      ...
      for (let n = 0; n < this.tableData.length; n++) {
        this.tableSelect.push({ patrolCycle: '', patrolType: '', lastPlanTime:'' })
      }
    },
