...
export default {
  data() {
    return {
      ...
      type: this.$dl+"终端,"+this.$dl+"本体,"+this.$dl+"接头,"+this.$dl+"分支箱,交叉互联箱,"+this.$dl+"接地箱"
    };
  },
  mounted() {},
  methods: {
    ...
    renderContent() {
      ...
      info
        .getList("/cable/failureLogging/selectBJNum", {
          begin: begintime,
          end: endtime,
          types: this.type
        })
    ...
    renderChart(res) {
      ...
      res.forEach(e => {
        ...
        for (let k of e.value) {
          if (k.name === this.$dl+"终端") {
            this.chart.yAxisData[0].data.push(k.count);
          }
          if (k.name === this.$dl+"本体") {
            this.chart.yAxisData[1].data.push(k.count);
          }
          if (k.name === this.$dl+"接头") {
            this.chart.yAxisData[2].data.push(k.count);
          }
          if (k.name === this.$dl+"分支箱") {
            this.chart.yAxisData[3].data.push(k.count);
          }
          if (k.name === "交叉互联箱") {
            this.chart.yAxisData[4].data.push(k.count);
          }
          if (k.name === this.$dl+"接地箱") {
            this.chart.yAxisData[5].data.push(k.count);
          }
        }
      });
      ...
      this.chart.dom.setOption({
        ...
        series: [
          ...
          {
            name: this.$dl+"分支箱",
            data: this.chart.yAxisData[3].data
          },
          {
            name: "交叉互联箱",
            data: this.chart.yAxisData[4].data
          },
          {
            name:this.$dl+"接地箱",
            data: this.chart.yAxisData[5].data
          },
        ]
      });
    },
